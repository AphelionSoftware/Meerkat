...
/// <reference path="datajs-1.1.0.js" />
...
(function () {
    ...
    function getClientParameter(parameterName) {
        ...
        var cookieString, parametersCookieValue, newParameterValues, newParameterName;
        ...
        if (parametersCookieValue) {
            newParameterValues = getAllCookieValues(parametersCookieValue, "&");
            for (newParameterName in newParameterValues) {
                clientParameters[newParameterName] = newParameterValues[newParameterName];
            }

            cookieExpirationDate = new Date();
            cookieExpirationDate.setTime(cookieExpirationDate.getTime() - 60000);
            document.cookie = clientParametersCookieName + "=; expires=" + cookieExpirationDate.toUTCString() + "; path=/";
        }
...
(function () {
    ...
    readyPromise = new WinJS.Promise(function (complete, error) {
        ...
        function getCultureInfoUri(language) {
            return "//ajax.aspnetcdn.com/ajax/globalize/0.1.1/cultures/globalize.culture." + language + ".js";
        }
...
(function () {
    ...
    function getLocalizedDisplayName(item) {
        ...
        return msls_getLocalizedString(item.displayName);
    };
...
(function () {
    ...
    function refreshNavigationPropertyQuery(
        ...
        if (isVirtual) {
            ...
            toKeyProperties.forEach(function (toKeyProperty, index) {
                ...
                primitiveType = msls_getUnderlyingTypes(fromKeyProperty.propertyType).primitiveType;
    ...
    function getNavigationPropertyTargetMultiplicity(data) {
        ...
        return associationToEnd.multiplicity;
    }
    ...
    function getIsLoaded(details, data) {
        ...
        if (data.state === navigationPropertyState.notInitialized) {
            ...
            if (getNavigationPropertyTargetMultiplicity(data) !== "Many" &&
...
(function () {
    ...
    function getEmailAddressExpressions() {
        var expressions = [],
            ...
            quotedLocal = "(" + start + "\"([\\sA-z0-9!#$%&'*+\\/=?\\^_`{|}~\\-\\(\\),:;<>@\\.]+|(\\\\\\\")+)+\")",
            localParts = "(" + quotedLocal + "|(" + start + "[A-z0-9!#$%&'*+\\/=?\\^_`{|}~\\-](\\.?[A-z0-9!#$%&'*+\\/=?\\^_`{|}~\\-]+)*))",
            namedDomainPart = "([A-z0-9]([A-z0-9\\-]{0,61}[A-z0-9])?\\.)+([A-z][A-z0-9]+|[A-z0-9]+[A-z]|([A-z0-9]+[A-z\\-]+[A-z0-9]?)+[A-z0-9]+))",
            ipAddressDomainPart = "(\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\]";

        expressions[0] = new RegExp(localParts + "@(" + namedDomainPart + "|" + ipAddressDomainPart + ")" + end);
        expressions[1] = new RegExp(localParts + end);
        expressions[2] = new RegExp(start + "(" + namedDomainPart + "|" + ipAddressDomainPart + ")" + end);
    ...
    function validateEmailAddress(modelItem, property, id, value) {
        ...
        if (!!value && !!validationAttribute && !expressions[0].test(value)) {
            if (!expressions[1].test(value)) {
                isValid = false;
            } else if (validationAttribute.domainRequired) {
                if (!propertiesAttribute || (
                    !!propertiesAttribute.defaultDomain &&
                    !expressions[2].test(propertiesAttribute.defaultDomain))) {
                    isValid = false;
                }
            }
        }
...
(function () {
            ...
            OData.read(
                ...
                operation.code(function error(err) {
                    ...
                    if (statusCode === 404) {
                        ...
                        serverErrors = tryGetServerErrors(err);
                        if (serverErrors) {
            ...
            function _requestUri_get() {
                ...
                if (current._queryParameters) {
                    $.each(current._queryParameters, function (key, val) {
                        if (val !== "null") {
                            options.push(key.toString() + "=" + encodeURIComponent(val.toString()));
                        }
                    });
                }
                if (filters.length > 0) {
    ...
    function initSaveChanges(dataService, operation) {
        ...
        changes.forEach(function (entity) {
            ...
            headers.accept = "application/atomsvc+xml;q=0.8, application/json;odata=verbose;q=0.5, */*;q=0.1";
...
(function () {
    ...
    function compareAndSetValue(me, object, propertyName, newValue) {
        ...
        if (!isSameValue && !!oldValue && !!newValue && oldValue.valueOf && oldValue.valueOf === newValue.valueOf) {
            isSameValue = newValue.valueOf() === oldValue.valueOf();
        }
        if (!isSameValue || !me._isTargetInitialized) {
            me.value = newValue;
    ...
    function convertFromString(stringValue, propertyDefinition) {
        ...
        if (primitiveTypeId === ":Byte" || primitiveTypeId === ":Int16" || primitiveTypeId === ":Int32" || primitiveTypeId === ":Int64") {
            ...
            var expressions = msls_getEmailAddressExpressions(),
                ...
                localPart = stringValue.replace(/@\s*$/, "");
            if (!expressions[0].test(localPart) &&
                !!attribute && !!attribute.defaultDomain &&
                expressions[1].test(localPart) &&
                expressions[2].test(attribute.defaultDomain.replace(/^\s*@/, ""))) {
...
(function () {
    ...
    function _initialize(initialData) {
        ...
        me.kind = model.kind;
        me.name = model.name;
        me.displayName = msls_getLocalizedDisplayName(model);
...
(function () {
    ...
    function initScreen(screen, dataWorkspace, modelId, screenParameters) {
        ...
        if (model) {
            screen.details.displayName = msls_getLocalizedDisplayName(model);
        }
...
(function () {
    ...
    function closePopup() {
        this.shellView.closePopup();
    }

    function showTab(tabName, boundaryOption, beforeShown) { ////TODO,stephwe remove
...
(function () {
    ...
    function loadMore(independent) {
        ...
        msls_promiseOperation(function (operation) {
            ...
            loaderPromise = me._activeLoaderPromise = loader.loadNext(me._data);
            loaderPromise._thenEx(function (error, items) {
                if (me._activeLoaderPromise === loaderPromise) {
...
var
    ...
    msls_task = "msls-task",
    msls_id_end_of_dialog = "msls-id-end-of-dialog",
...
(function () {
    ...
    function ScrollHelper(element) {
        ...
        scrollElement = scrollElement.length ? scrollElement : $(window);
...
(function () {
    ...
    msls_handleDialogFocus =
    function handleDialogFocus(dialog, bottomId) {
        var $bottomElement = $("#" + bottomId, dialog);

        dialog.focus();

        if (dialog[0]._keyDownEventHooked) {
            return;
        }

        dialog.keydown(function (e) {
            if (e.keyCode === $.mobile.keyCode.TAB && e.shiftKey) {
        ...
        $bottomElement.keydown(function (e) {
            if (e.keyCode === $.mobile.keyCode.TAB && !e.shiftKey) {
                dialog.focus();
                e.preventDefault();
            }
...
(function () {
    ...
    (function () {
        ...
        function _fillTemplate(view, contentItem, templateData) {
            ...
            $('<div class="msls-text-container"><span class="id-element"></span></div>').appendTo(view);
    ...
    (function () {
        ...
        function _fillTemplate(view, contentItem, templateData) {
            ...
            textElement.parent().addClass("msls-textBox-container");
        ...
        function _attachViewCore(templateData) {
            ...
            me._textElement.keydown(function (e) {
                if (e.keyCode === $.mobile.keyCode.ENTER) {
                    me._textElement.parent().focus();
                    me._textElement.focus();
                }
            });
...
(function () {
    ...
    function _addAttachedLabel(view, contentItem, templateData) {
        ...
        if (needLabelId) {
            ...
            contentHtml += "<label class='msls-label-text'>" + contentItem.displayName + "</label>";
            if (isHidden || isNone) {
                contentHtml += "</div>";
            }
        } else {
    ...
    function _addControlClasses(view, contentItem, contentContainer) {
        ...
        if (!_isUsingGroupControl(contentItem) && !_isUsingCollectionControl(contentItem)) {
...
(function () {
    ...
    function _buildMonthCalendar(me, firstDayOfMonth, daysInMonth) {
        var monthCalendar,
            ...
            firstDayOfWeek = standard.firstDay,
            days = standard.days,
        ...
        if (!monthCalendarTemplate) {
            ...
            for (day = 1; day <= 31; day++) {
                var dayNumberString = _padStringWithLeadingZeros(day.toString(), 2),
                    daysOfWeekIndex = (firstDayOfMonth + day - 1 + firstDayOfWeek) % daysOfWeekAbbreviated.length,
    ...
    function _buildDropdownElement(nameAttribute, options) {
        ...
        var selectHtml = '<select class="id-element" name="' + nameAttribute + '" data-icon="false" data-mini="true" style="min-width: 100%; max-width: none;">';
        if (!!options) {
    ...
    function _buildWrappedPicker(orderedDropdownArray, dropdownContainer) {
        ...
        dropdownHtml = '<fieldset data-role="controlgroup" data-type="horizontal" data-inline="true">' + orderedDropdownArray.join("") + "</fieldset>";
...
(function () {
    ...
    function _fillTemplate(view, contentItem, templateData) {
        $('<div class="msls-text-container"><a class="id-element ui-link" target="_blank"></a></div>')
            .appendTo(view);
        templateData.idElement = msls_getTemplateItemPath(view, msls_control_find(view, ".id-element"));
    }
...
(function () {
    ...
    function _fillTemplate(view, contentItem, templateData) {
        ...
        var template = "<select data-role='slider'><option value='false'>" + leftLabel + "</option><option value='true'>" + rightLabel + "</option></select>",
...
(function () {
    ...
    itemHtmlSelector = "li[data-msls='true']",
    itemDataKey = "__entity",
    listViewTemplate =
        '<div class="msls-vauto ' + msls_control_header + '"/>' +
        '<div{0}><ul class="msls-listview" data-role="listview" data-inset="false" /></div>',
    listViewEmptyHtml,
    ...
    function updateLoadingIndicator(listView, needShow) {
        var loadingIndicator = listView._loadingIndicator;
        if (needShow) {
            if (!loadingIndicator) {
    ...
    function loadMoreEntities(listView) {
        ...
        if (itemsCount < collectionData.length) {
            ...
            updateLoadingIndicator(listView, false);
            endLoading(listView);
        }
    }
    ...
    function addListItems(listView, items, startingIndex, skipTryLoadMore) {
        ...
        if (lastIndex >= 0) {
            ...
            if (!rowTemplate) {
                ...
                li.attr("data-msls", "true");
                li.addClass("ui-li ui-btn ui-btn-up-a");
    ...
    function joinVisualCollectionExecution(listView) {
        ...
        updateLoadingIndicator(listView, true);
        notifyLoading(listView, true);

        if (collectionState === _VisualCollectionState.loading) {
            (listView._collectionPromise = visualCollection.load(true)).then(
            ...
            (listView._collectionPromise = visualCollection.loadMore(true)).then(
                function success(result) {
    ...
    msls_defineClass("ui.controls", "ListView", function ListView(view) {
        ...
        _attachViewCore: function (templateData) {
            ...
            if (listView._isTileList && isDynamicTile) {
                listView._layoutUpdatingCallback = function (e) {
                    ...
                    if ($pageBeingUpdated.is($owningPage)) {
        ...
        _onBeforeTap: function onBeforeTap(e) {
                ...
                if (element !== ulHtmlElement) {
                    ...
                    if (element.getAttribute("data-msls") === "true") {
...
(function () {
    ...
    function showModalView(
        ...
        function buildContent() {
            ...
            $("<div id='" + msls_id_end_of_dialog + "' tabIndex='0'></div>").appendTo(dialog);
        ...
        function onPopupAfterOpen() {
            ...
            msls_handleDialogFocus(dialog.parent(), msls_id_end_of_dialog);
        ...
        function showPopup() {
            dialog
                ...
                .popup(
                {
                    overlayTheme: cssDefaultJqmTheme,
                    positionTo: "window",
                    theme: cssDefaultJqmTheme
                })
...
(function () {
    ...
    function _fillTemplate(view, contentItem, templateData) {
        ...
        var element = $('<a class="id-element" tabIndex="0" data-role="button" data-icon="arrow-d" data-mini="true" data-iconpos="right" data-theme="' + cssDefaultJqmTheme + '" />').appendTo(view),
    ...
    function showPicker(me) {
        ...
        msls_modalView.show({
            buildContentCallback: function buildContent(content) {
                ...
                $header.append($('<div class="msls-control-header"/>')
                    .append($('<div class="msls-text-container"/>')
...
(function () {
    ...
    function _fillTemplate(view, contentItem, templateData) {
        ...
        $('<div class="msls-text-container"><a class="id-element"></a></div>').appendTo(view);
        templateData.idElement = msls_getTemplateItemPath(view, msls_control_find(view, ".id-element"));
    }
...
(function () {
    ...
    function _parseUrl(me, url) {
        ...
        var pageId = url.replace(/^[^#]*#/, "").replace(/&.*$/, "") || me._firstPageId;
    ...
    function _handlePageChange(me, e, navigationData) {
        ...
        if (!!activePage && activePage.hasClass(msls_dialog)) {
            msls_handleDialogFocus(activePage, msls_id_end_of_dialog);
        }
    ...
    function _createUiForNavigationUnit(me, viewInfo) { ////TODO,stephwe: not use the term "page"?
    ...
    function _setPageIds($page, pageId) {
        ...
        $page.attr("id", pageId);
        $page.attr("data-url", pageId); ////TODO,stephwe: ?
    }

    function _createScreenFrameHeader(me, navigationUnit) {

        var $header = $("<div class='msls-task msls-header' data-role='header' data-position='fixed' data-update-page-padding='false' data-tap-toggle-blacklist='" + jqmToggleIgnoreList + "'></div>"),
    ...
    function _createTemplateForDialogFrame_helper(templateDataOut) {
        ...
        var outerFrame = $("<div class='msls-dialog-frame'></div>").appendTo(dialogPage),
            innerFrame = $("<div class='msls-dialog-inner-frame ui-body-" + cssDefaultJqmTheme + " msls-rows-layout + msls-fixed-height'></div>").
                appendTo(outerFrame);
        ...
        $("<div id='" + msls_id_end_of_dialog + "' tabIndex='0'></div>").appendTo(dialogPage);
    ...
    function _createTemplateForPopupFrame(popupContentItem, templateDataOut) {
        ...
        var $frame = $(msls_templateStrings[popupTemplateId])
            .attr("data-theme", cssDefaultJqmTheme);
    ...
    function _createPopup(me, popupId, $page, rootContentItem) {
        ...
        $("<div id='" + msls_id_end_of_dialog + "' tabIndex='0'></div>").appendTo($popup);
    ...
    function showPopup(rootContentItem, afterClosed, options) {
        ...
        return new WinJS.Promise(function initShowPopup(complete, error) {
            ...
            var $popups = $(".msls-popup", $.mobile.activePage),
                popupId,
                $popup;
            ...
            $(window).one("popupafteropen", function (evt) {
                ...
                complete();
                msls_handleDialogFocus($popup.parent(), msls_id_end_of_dialog);
