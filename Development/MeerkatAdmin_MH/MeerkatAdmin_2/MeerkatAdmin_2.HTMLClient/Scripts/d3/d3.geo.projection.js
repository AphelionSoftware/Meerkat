(function() {
  ...
  function armadillo(φ0) {
    ...
    function forward(λ, φ) {
      ...
      return [ (1 + cosφ) * Math.sin(λ), k + Math.sin(φ) * cosφ0 - (1 + cosφ) * sinφ0 * cosλ ];
    }
    forward.invert = function(x, y) {
      var λ = 0, φ = 0, i = 50;
      do {
  ...
  function armadilloProjection() {
    var φ0 = π / 9, sφ0 = φ0 > 0 ? 1 : -1, tanφ0 = Math.tan(sφ0 * φ0), m = projectionMutator(armadillo), p = m(φ0);
    ...
    function clip(p) {
      var clipPolygon = [], λ1 = sφ0 * (1 - 1e-6), φ1 = sφ0 * (90 - 1e-6);
