...
myapp.AddEditProjectParticipants.created = function (screen) {
    ...
    //Set the programme - it's prefiltered by parameter.
    myapp.activeDataWorkspace.MeerkatData.Programmes_SingleOrDefault(screen.Programme_ID).execute().then(function (Programme) {
        screen.PeopleReachedValue.setProgramme(Programme.results[0]);
    });

    //Default the version to publish
    myapp.activeDataWorkspace.MeerkatData.DataVersions_SingleOrDefault(1).execute().then(function (ver) {
        screen.PeopleReachedValue.setDataVersion(ver.results[0]);
    }
    );
...
myapp.AddEditProjectParticipants.Programme_postRender = function (element, contentItem) {
    var input = $(element);
    //Setting here as we need access to the control
    contentItem.dataBind("screen.ProjectsSorted.selectedItem", function (newValue) {

        if (newValue) {
            contentItem.screen.PeopleReachedValue.setProgramme(null);
...
myapp.AddEditProjectParticipants.Project_postRender = function (element, contentItem) {
    ...
    contentItem.dataBind("screen.ProgrammeSorted.selectedItem", function (newValue) {
        ...
        if (newValue) {
            contentItem.screen.PeopleReachedValue.setProject(null);
            input.find("input").val("");
...
myapp.AddEditProjectParticipants.Sector_postRender = function (element, contentItem) {
    var input = $(element);
    contentItem.dataBind("screen.SubSectorSorted.selectedItem", function (newValue) {
        if (contentItem.screen.Client && contentItem.screen.Client.Value == "CARE Somalia") {
            ...
            if (newValue) {
                contentItem.screen.PeopleReachedValue.setSector(null);
                input.find("input").val("");
...
myapp.AddEditProjectParticipants.SubSector_postRender = function (element, contentItem) {
    var input = $(element);
